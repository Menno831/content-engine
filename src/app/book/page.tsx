...
export default function BookPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // TODO: Vervang met Cal.com embed of API route
    const subject = encodeURIComponent(`Strategiegesprek aanvraag — ${form.company || form.name}`);
    ...
    <main className="min-h-screen grid-bg flex flex-col">
      ...
      <nav className="fixed top-0 left-0 w-full z-50 backdrop-blur-xl bg-background/60 border-b border-white/[0.04]">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6 md:px-12 lg:px-20 h-16">
          ...
          <a href="/" className="text-sm text-muted hover:text-foreground transition-colors">&larr; Terug naar homepage</a>
        </div>
      </nav>

      {/* Content */}
      <div className="flex-1 px-6 md:px-12 lg:px-20 pt-24 pb-12">
        <div className="max-w-5xl mx-auto grid md:grid-cols-12 gap-10 items-start">
          ...
          <div className="md:col-span-5">
            ...
            <h1 className="font-display text-3xl md:text-4xl font-extrabold leading-[0.95] tracking-tight">
              Plan je gratis
              <br />
              <span className="text-accent">Strategiegesprek</span>
            </h1>
            ...
            <p className="mt-6 text-sm text-muted">
              ...
              <br />
              Wil je liever direct mailen?{" "}
              <a href="mailto:hallo@contentengine.nl" className="text-accent hover:underline">hallo@contentengine.nl</a>
            </p>
          </div>
          ...
          <div className="md:col-span-7">
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className="p-8 rounded-2xl border border-card-border bg-card/60 space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="name" className="block text-sm font-medium mb-1.5">Naam *</label>
                    <input type="text" id="name" name="name" required value={form.name} onChange={handleChange} placeholder="Je volledige naam"
                      className="w-full px-4 py-3 rounded-lg bg-background border border-card-border text-foreground placeholder:text-muted focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-colors" />
                  </div>
                  <div>
                    <label htmlFor="email" className="block text-sm font-medium mb-1.5">Email *</label>
                    <input type="email" id="email" name="email" required value={form.email} onChange={handleChange} placeholder="naam@bedrijf.nl"
                      className="w-full px-4 py-3 rounded-lg bg-background border border-card-border text-foreground placeholder:text-muted focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-colors" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="phone" className="block text-sm font-medium mb-1.5">Telefoonnummer</label>
                    <input type="tel" id="phone" name="phone" value={form.phone} onChange={handleChange} placeholder="06-12345678"
                      className="w-full px-4 py-3 rounded-lg bg-background border border-card-border text-foreground placeholder:text-muted focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-colors" />
                  </div>
                  <div>
                    <label htmlFor="company" className="block text-sm font-medium mb-1.5">Bedrijf / Website</label>
                    <input type="text" id="company" name="company" value={form.company} onChange={handleChange} placeholder="bedrijfsnaam.nl"
                      className="w-full px-4 py-3 rounded-lg bg-background border border-card-border text-foreground placeholder:text-muted focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-colors" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea id="message" name="message" rows={3} value={form.message} onChange={handleChange} placeholder="Wat voor bedrijf run je? Wat is je grootste uitdaging met content?"
                    className="w-full px-4 py-3 rounded-lg bg-background border border-card-border text-foreground placeholder:text-muted focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-colors resize-none" />
                </div>
                ...
                <button type="submit" disabled={sending}
                  ...
                  {sending ? "Bezig..." : "Plan mijn Strategiegesprek"}
                  {!sending && (
      ...
      {/* Footer */}
      <footer className="border-t border-white/[0.06] px-6 md:px-12 lg:px-20 py-8 mt-auto">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <a href="/" className="font-display text-sm font-extrabold tracking-tight">
            CONTENT<span className="text-accent">ENGINE</span>
          </a>
          <div className="flex items-center gap-4 text-sm text-muted">
            <a href="mailto:hallo@contentengine.nl" className="hover:text-accent transition-colors">hallo@contentengine.nl</a>
            <span className="text-card-border">|</span>
            <a href="/" className="hover:text-accent transition-colors">Homepage</a>
          </div>
        </div>
      </footer>
    </main>
